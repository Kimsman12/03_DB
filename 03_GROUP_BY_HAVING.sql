...
-- 직급코드, 급여평균, 인원 수 를 직급코드 오름차순으로 조회
...
-- EMPLOYEE 테이블에서 성별(남/여)과 각 성별 별 인원수, 급여 합을
-- 인원 수 오름차순으로 조회
...
GROUP BY DEPT_CODE; -- 부서의

-- EMPLOYEE 테이블에서
-- 2000년도 이후 입사자들의 직급별 급여합을 조회

SELECT JOB_CODE, SUM(SALARY)
FROM EMPLOYEE
WHERE 
-- 1) HIRE_DATE >= TO_DATE('2000-01-01')
-- 2) EXTRACT (YEAR FROM HIRE_DATE) >= 2000
...
-- *** GROUP BY 사용시 주의사항 ***
--> SELECT 문에 GROUB BY절을 사용할 경우
-- SELECT절에 명시한 조회하려는 컬럼 중
-- 그룹 함수가 적용되지 않은 컬럼을
-- 모두 GROUP BY절에 작성 해야함.

-- EMPLOYEE 테이블에서 부서별로 같은 직급인 사원 수를 조회
-- 부서코드 오름차순, 직급코드 내립차순으로 조회
-- 부서코드, 직급코드, 인원수

SELECT DEPT_CODE, JOB_CODE, COUNT(*)
FROM EMPLOYEE
GROUP BY DEPT_CODE, JOB_CODE -- DEPT_CODE 로 그룹을 나누고, 나눠진 그룹 내에서 JOB_CODE로 그룹을 분류
...
-- 부서코드, 평균급여 조회 부서코드 오름차순으로 정렬

SELECT DEPT_CODE, ROUND(AVG(SALARY))
FROM EMPLOYEE
GROUP BY DEPT_CODE
HAVING AVG(SALARY) >= 3000000 --> DEPT_CODE 그룹 중 평균 급여가 3백만원 이상인 그룹만 조회
ORDER BY DEPT_CODE;

-- EMPLOYEE 테이블에서 직급별 인원수가 5명 이하인
-- 직급코드, 인원수 조회 (직급코드 오름차순 정렬)
...
-- QL Error [1789] [42000]: ORA-01789: 질의 블록은 부정확한 수의 결과 열을 가지고 있습니다.
