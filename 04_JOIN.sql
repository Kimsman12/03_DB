...
-- ANSI : 연결에 사용할 컬럼명이 다를경우 ON()을 사용
SELECT EMP_ID, EMP_NAME, DEPT_CODE, DEPT_TITLE
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID);
...
-- EMPLOYEE, JOB 테이블 참조하요
-- 사번, 이름, 직급코드, 직급명 조회

SELECT * FROM JOB;

-- ANSI에서 연결에 사용할 컬럼명이 같은 경우 USING(컬럼명)을 사용함
...
-- ** 오라클 구문에서 FULL OUTER JOIN 사용 불가
...
-- 테이블마다 별칭 작성(미작성시 열의 정의가 애매하다 오류 발생)
...
-- 반드시 두 테이블간에 동일한 컬럼명, 타입을 가진 컬럼이 필요
--> 없을 경우 교차조인
